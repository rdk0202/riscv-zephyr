		...
		s16_t *raw_avg)
{
	...
	/* Return code for functions which return error codes */
	int rc = 0;

	/* Accumulator for the raw sample value */
	s32_t sum = 0;
	...
	for(int i = 0; i < count; i++)
	{
		...
		if(rc < 0)
		{
			SYS_LOG_DBG("Failed to fetch sample");
			return -EIO;
		}
...
static int lsm303c_accel_selftest(struct device *dev) {
	int rc = 0;
	s16_t raw_avg_ctrl[3], raw_avg_st[3];
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Failed to enable accel self-test");
		return -EIO;
	}
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Failed to disable accel self-test");
		return -EIO;
	}
	...
	{
		/* Calculate delta */
		s16_t delta = raw_avg_st[i] - raw_avg_ctrl[i];

		/* Convert delta to value */
		struct sensor_value val;
		lsm303c_accel_convert(&val, delta,
			LSM303C_DEFAULT_ACCEL_FULLSCALE_FACTOR);

		/* Check delta is between 686,465 um/s^2 and 14,709,975 um/s^2 */
		s32_t mag = val.val1 * 1000000 + val.val2;
		if((mag < 686465) || (mag > 14709975)) {
			SYS_LOG_DBG("Magnitude outside of tolerances: %d", mag);
			return -EIO;
		}
	}
...
static int lsm303c_magn_selftest(struct device *dev) {
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Failed to enable magn self-test");
		return -EIO;
	}
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Failed to disable magn self-test");
		return -EIO;
	}
	...
	for(int i = 0; i < 3; i++)
	{
		...
		/* Convert delta to value */
		struct sensor_value val;
		lsm303c_magn_convert(&val, delta,
		...
		if(i < 2)
		{
			if((mag > -1000000) || (mag < -3000000)) {
				SYS_LOG_DBG("X/Y Magnitude outside of tolerances: %d", mag);
				return -EIO;
			}
		} else /* Z should deflect by -0.1 to -1 gauss */
		{
			if((mag > -100000) || (mag < -1000000)) {
				SYS_LOG_DBG("Z Magnitude outside of tolerances: %d", mag);
				return -EIO;
			}
		}
	}
...
int lsm303c_selftest(struct device *dev)
{
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Accelerometer self-test failed");
		return rc;
	}
	...
	if(rc < 0)
	{
		SYS_LOG_DBG("Magnetometer self-test failed");
		return rc;
	}
