...
#define LOG_LEVEL CONFIG_LOG_SENSOR_LEVEL
#include <logging/log.h>
LOG_MODULE_REGISTER(liteon_ltr329);
			       ...
			       struct sensor_value *val)
{
	...
	case 0:
		...
		value = ((s64_t) sample) * 96LL * 200LL * 1000000LL /
			 (1190LL * gain);
		break;
	case 1:
...
static int ltr329_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_IR:
		...
		if (i2c_reg_read_byte(data->i2c_master,
				      ...
				      REG_ALS_DATA_CH1_0,
				      &reg) < 0) {
			LOG_DBG("Failed to read 0x%02X@0x%02X",
				REG_ALS_DATA_CH1_0,
				config->i2c_slave_addr);
			return -EIO;
		}
...
static int ltr329_init(struct device *dev)
{
	...
	if (i2c_reg_read_byte(data->i2c_master,
			      ...
			      REG_PART_ID,
			      &part_id) < 0) {
		LOG_DBG("Failed to read part id");
		return -EIO;
	}
	if (part_id != VAL_PART_ID) {
		LOG_DBG("Invalid part ID 0x%x", part_id);
		return -EIO;
	}
	LOG_DBG("LTR329 Part ID: 0x%0x", part_id);

	if (i2c_reg_read_byte(data->i2c_master,
			      config->i2c_slave_addr,
			      REG_MANUFAC_ID,
			      &manufac_id) < 0) {
		LOG_DBG("Failed to read manufacturer id");
		return -EIO;
	}
	if (manufac_id != VAL_MANUFAC_ID) {
		LOG_DBG("Invalid manufacturer ID 0x%x", manufac_id);
		return -EIO;
	}
	LOG_DBG("LTR329 Manufacturer ID: 0x%0x", manufac_id);
	...
	if (i2c_reg_update_byte(data->i2c_master,
		...
		LOG_DBG("Failed to set integration rate");
		return -EIO;
	}
	if (i2c_reg_update_byte(data->i2c_master,
				config->i2c_slave_addr,
		...
		LOG_DBG("Failed to set sample rate");
		return -EIO;
	}
...
const struct ltr329_config ltr329_config = {
	.i2c_master_dev_name = DT_LTR329_I2C_MASTER_DEV_NAME,
	.i2c_slave_addr = DT_LTR329_I2C_ADDR,
};

static struct ltr329_data ltr329_data;

DEVICE_AND_API_INIT(ltr329, DT_LTR329_DEV_NAME, ltr329_init,
